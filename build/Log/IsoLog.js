  ...
  (0, _createClass2.default)(Log, [{
    ...
    value: function doLog(level, args) {
      ...
      if (this.levels[level] && this.levels[level].i >= this.levels[this.level].i) {
        ...
        var thingType = (0, _typeof2.default)(thingToLog);
      ...
      } else {// console.log('**** LOG LEVEL NOT MET');
      }
    }
  }, {
    ...
    value: function getDatetimeString() {
      ...
      var nowStr = "".concat(year, "-").concat(month, "-").concat(day, " ").concat(hour, ":").concat(minute, ":").concat(second, ":").concat(millisecond);
      return nowStr;
    }
  }, {
    key: "colorize",
    value: function colorize(level, str, bold) {
      ...
      if (CLIENT) {
        ...
        if (this.levels[level].bgHex) {
          style += "background: ".concat(this.levels[level].bgHex, ";");
        }

        if (this.levels[level].hex) {
          style += "color: ".concat(this.levels[level].hex, ";");
        }
        ...
        colorizedStr = ["%c".concat(str), style];
      } else {
        // if (chalk.hex) {
        // 	if (this.levels[level].bgHex) {
            ...
            if (matches2 && matches2[1]) {
              ...
              _this2.getSource({
                ...
                if (original) {
                  ...
                  callerFunc = "".concat(originalFile, ":").concat(original.line, ":").concat(original.column);
                  return resolve(callerFunc);
                }

                callerFunc = "".concat(matches2[1], " | ").concat(matches2[2]);
                return resolve(callerFunc);
              });
            } else {
              return resolve();
    ...
    value: function getSource(_ref) {
      ...
      return new Promise(function (resolve) {
        var fullSource = "".concat(sourceRoot).concat(sourceFile);
        var fullMapSource = "".concat(sourceRoot).concat(mapFile);
